% =========================================================================
% runme_shakti_newmesh_cluster.m
%
% Cluster runner for the RecoveryNewMesh SHAKTI transient simulation.
%
% Put this script in:
%   /share/home/u10109/data/mumu/ISSM-Linux-MATLAB_1773134435/SHAKTI
%
% Required input model:
%   Models/RecoveryNewMesh_Hydrology_ShelfGroundedRingNoSlidingCoupled.mat
%
% Main output:
%   Results/RecoveryNewMesh_SHAKTI_<start>d_to_<end>d_<dt>s_<tag>/
%   Results/.../segments/RecoveryNewMesh_SHAKTI_<seg_start>d_to_<seg_end>d_<dt>s_<tag>.mat
% =========================================================================
clear; clc; close all;
...
% --- Server paths ---
workdir     = '/share/home/u10109/data/mumu/ISSM-Linux-MATLAB_1773134435/SHAKTI';
...
input_model_file = fullfile(workdir, 'Models', 'RecoveryNewMesh_Hydrology_ShelfGroundedRingNoSlidingCoupled.mat');
runs_root        = fullfile(workdir, 'Results');

% --- Simulation settings ---
sim_dt_seconds  = 1800;            % 30 min
sim_n_steps     = 2 * 24 * 360;    % 4*360 days at 30 min
sim_output_freq = 24 * 2 * 4;              % Save every 4 days
checkpoint_save_days = 180;         % Save a MAT checkpoint every 30 model days
sim_start_day = NaN;          % NaN = infer from input model's last saved time.
hydro_relaxation = 1;       % SHAKTI nonlinear under-relaxation. Use NaN to keep input model value.
run_tag = 'np15';             % Optional, e.g. 'np16'. Empty = auto Slurm/script tag.
...
if ~strcmp(getenv('RUNME_SHAKTI_NEWMESH_BOOTSTRAPPED'), '1')
    ...
    if can_use_current_matlab(issm_root, issm_src, issm_prefix)
        setenv('RUNME_SHAKTI_NEWMESH_BOOTSTRAPPED', '1');
    else
        ...
        wrapper_sh = fullfile(workdir, ...
            ['bootstrap_shakti_newmesh_' datestr(now, 'yyyymmdd_HHMMSSFFF') '.sh']);
        ...
        fprintf(fid, 'export RUNME_SHAKTI_NEWMESH_BOOTSTRAPPED=1\n');
        fprintf(fid, 'cd %s\n', shell_quote(workdir));
...
input_model_tag = local_model_tag_from_file_name(input_model_file);
...
sim_result_base = sprintf('%s_SHAKTI_%s', input_model_tag, sim_period_tag);
...
logmsg('============================================================');
logmsg('New-mesh SHAKTI transient simulation started');
logmsg('workdir     : %s', workdir);
logmsg('model tag   : %s', input_model_tag);
...
while step_done < sim_n_steps
    ...
    segment_base_name = sprintf('%s_SHAKTI_%s', input_model_tag, segment_period_tag);
    if isempty(run_tag)
...
function tag = local_model_tag_from_file_name(model_file)
    [~, base] = fileparts(model_file);
...
function local_validate_hydrology_model(md, input_file)
    ...
    fprintf('spchead nodes: %d\n', sum(isfinite(md.hydrology.spchead(:))));
