%% plot_model_general.m
% General plotting script copied from plot_inversion.m.
% Only edit model_file and output_root below when switching to another
% model under outputs/models. The output folder no longer depends on L-curve
% folder names or w501 strings in the model filename.
%  批量输出反演结果�?(19 �?
...
output_root     = fullfile(paths.figures, 'Recovery_Inversion_generalplot');
...
% --- 是否启用回写并另�?synced 模型 ---
enable_sync_save = 0;
synced_model_file = '';   % empty: save as outputs/models/<input_model_name>_synced.mat
...
%   coeff101_name = '40000'
%   coeff103_name = '10'
% L-curve weight parsing is intentionally disabled in this general script.
% Set output_root manually above for models with arbitrary names.
...
% 自动生成输出目录
assert(~isempty(output_root), 'output_root must be set manually.');
outroot = output_root;
...
if enable_sync_save
    ...
    % --- 从文件名解析 w501 字符�?---
    % No filename weight parsing in this general script.

    % --- 另存 synced 模型 ---
    if isempty(synced_model_file)
    synced_path = fullfile(paths.models, [model_name '_synced.mat']);
    else
        synced_path = synced_model_file;
    end
